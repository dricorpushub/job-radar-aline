...
class RegrasFiltro:
    ...
    # Regras opcionais para perfis mais restritivos. Perfis antigos mantêm
    # o comportamento original quando estes campos ficam vazios/None.
    termos_bloqueados_titulo: list[str] | None = None
    modalidades_aceitas: list[str] | None = None
    niveis_senioridade_alvo: tuple[str, ...] = ("Júnior", "Pleno")
    niveis_senioridade_penalizados: tuple[str, ...] = (
        "Sênior", "Especialista", "Liderança",
    )
...
class Job:
    ...
    def _avaliar(self, regras: RegrasFiltro) -> _Avaliacao:
        ...
        titulo_bloqueado = bool(regras.termos_bloqueados_titulo) and any(
            _contem_termo(_normalizar(t), titulo_norm, aceitar_plural=True)
            for t in regras.termos_bloqueados_titulo
        )
        ...
        modalidade_aceita = True
        if regras.modalidades_aceitas is not None:
            modalidades_norm = {_normalizar(m) for m in regras.modalidades_aceitas}
            # Uma busca nativa remota já confirma a modalidade mesmo quando
            # o card não escreve "remoto" no campo de localização.
            modalidade_aceita = bate_remoto or modalidade_norm in modalidades_norm

        return _Avaliacao(
            aprovada=(
                bate_keyword and bate_cidade and modalidade_aceita
                and not titulo_bloqueado
            ),
    ...
    def pontuar_relevancia(self, regras: RegrasFiltro) -> int:
        ...
        if nivel in regras.niveis_senioridade_alvo:
            pontos_senioridade = _PESO_SENIORIDADE_ALVO
        elif nivel in regras.niveis_senioridade_penalizados:
